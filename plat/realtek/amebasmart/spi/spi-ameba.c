...
/* SPIC Driver */
static uint32_t spi_flash_setser(struct ameba_spi *dev, uint32_t ser_num)
{
 ...
 */
static uint32_t spi_flash_setbaudr(struct ameba_spi *dev, uint32_t baudrate)
{
	struct spi_flash_portmap *spi_flash_map;
 ...
 */
static uint32_t spi_flash_setfbaudr(struct ameba_spi *dev, uint32_t fbaudrate)
{
	struct spi_flash_portmap *spi_flash_map;
 ...
 */
static uint32_t spi_flash_set_dummy_cycle(struct ameba_spi *dev,
					uint32_t dum_cycle)
{
	struct spi_flash_portmap *spi_flash_map;
	uint32_t cycle;
...
static void set_auto_length(struct ameba_spi *dev)
{
 ...
 */
static void flash_init(struct ameba_spi *dev,
...
static void auto_init(struct ameba_spi *dev)
{
	struct spi_flash_portmap *spi_flash_map;
...
static void flash_read(struct ameba_spi *dev)
{

	volatile uint32_t *mem = (volatile uint32_t *)(PLAT_AMEBASMART_FIP_BASE);
...
{
	struct ameba_spi dws;
	struct spi_flash_param spi_flash_config;

	dws.regs = (struct ameba_spi *)(SPIC_BASE);
	dws.auto_regs = (struct ameba_spi *)(FLASH_BASE);
